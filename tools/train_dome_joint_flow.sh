...
CFG="${CFG:-./config/train_dome_joint_flow.py}"
WORK_DIR="${WORK_DIR:-./work_dir/dome_joint_flow}"
...
echo "[任务] Non-resample joint flow: occupancy latent + future trajectory"
echo "[配置] $CFG"
echo "[输出] $WORK_DIR"
...
if [[ ! -d "data/nuscenes" ]]; then
  ...
  echo "       可以用软链接或修改 config/train_dome_joint_flow.py 里的 data_path。"
  exit 1
fi
