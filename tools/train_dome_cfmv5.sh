...
CFG="${CFG:-./config/train_dome_cfmv5.py}"
WORK_DIR="${WORK_DIR:-./work_dir/dome-cfmv5}"
...
echo "[任务] Non-resample dome-cfmv5 joint flow: occupancy latent + trajectory tokens"
echo "[配置] $CFG"
echo "[输出] $WORK_DIR"
...
if [[ ! -d "data/nuscenes" ]]; then
  ...
  echo "       可以用软链接或修改 config/train_dome_cfmv5.py 里的 data_path。"
  exit 1
fi
